 ...
 */
#define MAP_HEADER(tbl) ((map_header *)((char *)(tbl) - sizeof(map_header)))
...
#define MAP_RESIZE(tbl, new_cap)                                                                                     \
    do {                                                                                                             \
        ...
        void *_ms_old_ptr = _ms_tbl ? ((char *)(_ms_tbl) - sizeof(map_header)) : NULL;                               \
        map_header *_ms_new_hdr = (map_header *)malloc(sizeof(map_header) + _ms_new_cap * _ms_elem_size);            \
        ...
        char *_ms_new_arr = (char *)_ms_new_hdr + sizeof(map_header);                                                \
        memset(_ms_new_arr, 0, _ms_new_cap * _ms_elem_size);                                                         \
        if (_ms_old_ptr) {                                                                                           \
            char *old_items = (char *)_ms_old_ptr + sizeof(map_header);                                              \
...
#define map_put(tbl, item)                                                                        \
    do {                                                                                          \
        ...
        if (!_mp_tbl) {                                                                           \
            ...
            size_t _mp_elem_size = sizeof(*(_mp_tbl));                                            \
            map_header *_mp_hdr = (map_header *)malloc(sizeof(map_header) +                       \
                                            _mp_cap * _mp_elem_size);                             \
            _mp_hdr->capacity = _mp_cap;                                                          \
            _mp_hdr->count = 0;                                                                   \
            _mp_hdr->load_factor = MAP_LOAD_FACTOR;                                               \
            _mp_hdr->growth_factor = MAP_GROWTH_FACTOR_DEFAULT;                                   \
            _mp_tbl = (void *)((char *)_mp_hdr + sizeof(map_header));                             \
...
#define map_get(tbl, key)                                                             \
    ({ __typeof__(tbl) _mg_tbl = (tbl);                                               \
       _mg_tbl ? _map_get_impl(_mg_tbl, (key), sizeof(*(_mg_tbl))) : (void *)0; })
...
#define map_set_min_capacity(tbl, min_cap)                                                              \
    do {                                                                                                \
        ...
        if (!_m_tbl) {                                                                                  \
            ...
            size_t _m_elem_size = sizeof(*(_m_tbl));                                                    \
            map_header *_m_hdr = (map_header *)malloc(sizeof(map_header) +                              \
                                        _m_cap * _m_elem_size);                                         \
            _m_hdr->capacity = _m_cap;                                                                  \
            _m_hdr->count = 0;                                                                          \
            _m_hdr->load_factor = MAP_LOAD_FACTOR;                                                      \
            _m_hdr->growth_factor = MAP_GROWTH_FACTOR_DEFAULT;                                          \
            _m_tbl = (void *)((char *)_m_hdr + sizeof(map_header));                                     \
...
/* ------------------------------------------------------------------
   ...
   The value is stored in the map's header and used for subsequent expansions.
   Example:
...
#define map_dup(tbl)                                                                        \
    ({ __typeof__(tbl) _orig = (tbl);                                                       \
       ...
       if (_orig) {                                                                         \
           ...
           map_header *_new_hdr = malloc(sizeof(map_header) + _cap * _elem_size);           \
           if (_new_hdr) {                                                                  \
               ...
               void *_new_arr = (char *)_new_hdr + sizeof(map_header);                      \
...
#define map_free(tbl)                                                                 \
    do {                                                                              \
       if (tbl) { free((char *)(tbl) - sizeof(map_header)); (tbl) = NULL; }           \
    } while (0)
